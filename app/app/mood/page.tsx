...
import { useAddMoodEntry, useMoodEntries, useUserStats } from "@/hooks/useQueries";
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      <motion.div
        ...
        style={{
          background: "radial-gradient(circle, oklch(0.84 0.065 144 / 0.8), oklch(0.921 0.04 286 / 0.5))",
      ...
      >
        <motion.div
          ...
          style={{ background: "radial-gradient(circle, oklch(0.921 0.04 286 / 0.9), oklch(0.888 0.071 53 / 0.6))" }}
        />
      </motion.div>
    ...
    <AnimatePresence>
      {stressReliefOpen && (
        <motion.div
          ...
          style={{ background: "oklch(0.18 0.02 260 / 0.92)", backdropFilter: "blur(16px)" }}
        >
          <motion.div
          ...
          >
            <div className="space-y-1">
              <h2 className="text-2xl font-bold text-foreground">{"You're safe here 💙"}</h2>
              <p className="text-sm text-muted-foreground leading-relaxed">
                {"Let's breathe together. Follow the circle and let tension melt away."}
              </p>
            </div>
            <BreathingCircle />
            <div className="glass rounded-2xl px-5 py-3 w-full">
              <p className="text-xs text-muted-foreground leading-relaxed">
                {"🤍 It's okay to feel overwhelmed. This moment will pass. You've handled tough moments before — you can do this too."}
              </p>
            </div>
            <button
              type="button"
...
function EmojiDot({ cx = 0, cy = 0, value = 3 }: EmojiDotProps) {
  const emoji = EMOJIS.find((e) => e.score === Math.round(value))?.emoji ?? "😐";
  return (
    <text x={cx} y={cy - 6} textAnchor="middle" fontSize={14} style={{ userSelect: "none" }}>
      {emoji}
    </text>
  );
}
  ...
  return (
    <>
      ...
      <div ref={containerRef} className="min-h-screen px-4 py-6 space-y-5 max-w-lg mx-auto pb-24">
        ...
        >
          <div>
            <h1 className="text-2xl font-bold text-foreground tracking-tight">How are you feeling?</h1>
            <p className="text-sm text-muted-foreground mt-0.5">Your daily emotional check-in 🌿</p>
          </div>
          <button
            type="button"
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-base font-semibold text-foreground/70">day streak</span>
                </div>
                <p className="text-xs text-foreground/60 ml-11">{"Keep going — you're doing amazing!"}</p>
              </div>
        ...
        >
          <p className="text-sm font-semibold text-muted-foreground">Tap an emoji to log your mood</p>
          ...
          <div className="flex justify-between gap-1">
            {EMOJIS.map((e) => (
              <button
                ...
                style={{
                  background: selected === e.score ? "oklch(var(--lavender) / 0.7)" : "oklch(var(--muted) / 0.5)",
                  transform: selected === e.score ? "scale(1.12)" : "scale(1)",
                  boxShadow: selected === e.score
              ...
              >
                ...
                <span
                  ...
                  style={{ color: selected === e.score ? "oklch(var(--foreground))" : "oklch(var(--muted-foreground))" }}
                >
                  {e.label}
                </span>
              </button>
          ...
          <AnimatePresence mode="wait">
            {submitted ? (
              ...
              >
                <p className="text-sm font-semibold text-foreground">✅ Mood logged! Keep it up 🌿</p>
                {lastScore !== null && (
                  <motion.p
              ...
              <motion.button
                ...
                whileHover={selected !== null ? { y: -2, boxShadow: "0 8px 24px oklch(var(--accent-coral) / 0.4)" } : {}}
                whileTap={{ scale: 0.98 }}
        ...
        >
          <h2 className="text-sm font-semibold text-foreground mb-4">Your week at a glance 📈</h2>
          {chartData.length > 0 ? (
            <>
              <svg width="0" height="0" style={{ position: "absolute" }} aria-hidden="true">
                <defs>
                  <linearGradient id="moodLineGradient" x1="0" y1="0" x2="1" y2="0">
                    <stop offset="0%" stopColor="oklch(0.72 0.05 286)" stopOpacity={1} />
                    <stop offset="100%" stopColor="oklch(0.80 0.06 144)" stopOpacity={1} />
                  </linearGradient>
                  <linearGradient id="moodFillGradient" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="oklch(0.72 0.05 286)" stopOpacity={0.25} />
                    <stop offset="100%" stopColor="oklch(0.80 0.06 144)" stopOpacity={0.02} />
                  </linearGradient>
                </defs>
              </svg>
              <ResponsiveContainer width="100%" height={180}>
                <AreaChart data={chartData} margin={{ top: 20, right: 4, left: -24, bottom: 0 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(v) => {
                      ...
                      const found = EMOJIS.find((e) => e.score === Math.round(score));
                      return [`${found?.emoji ?? "😐"} ${found?.label ?? score}`, "Mood"];
                    }}
                  />
                  <Area
                    type="monotone"
                    ...
                    dot={(props) => <EmojiDot {...props} />}
                    activeDot={{ r: 5, strokeWidth: 2, fill: "oklch(0.72 0.05 286)" }}
                  />
                </AreaChart>
              </ResponsiveContainer>
            </>
